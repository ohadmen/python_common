...
def apply_colormap(cm_name: str, v: np.ndarray) -> np.ndarray:
    """

    :param cm_name: color map name (e.g. 'Accent', 'Accent_r', 'Blues', 'Blues_r', 'BrBG',
    ...
    :param v: nd array of data of shape p (e.g. p=(m,n), p=m)
    :return:  input data, with rgb over the scalalr dimention (e.g. (m,n)->(m,n,3) (m,1)->(m,3) )
    :rtype: np.ndarray
    """
    cm_data = cm.get_cmap(cm_name, 256).colors[:, :3]
    v = v - np.min(v)
    v = (v / np.max(v) * 255).astype(int)
    v_rgb = cm_data[v]
    return v_rgb
